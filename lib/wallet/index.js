...
const { toBuffer, weiToEther } = require("../utils");
...
exports.deposit = async (
    ...
    new web3Instance.eth.Contract(exchangeAbi, exchangeAddress).methods
        .deposit(
            token.symbol === "ETH" ? "0x0" : token.address,
            weiToEther(new BigNumber(weiAmount)).toFixed()
        )
        .send({
            from: ethereumAccount,
            gas: 500000,
...
exports.createAccount = async (
    ...
    new web3Instance.eth.Contract(exchangeAbi, exchangeAddress).methods
        .createOrUpdateAccount(publicKeyX, publicKeyY, permission)
        .send({
            from: ethereumAccount,
            gas: 500000,
            gasPrice,
